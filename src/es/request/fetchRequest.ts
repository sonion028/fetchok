  ...
  do {
    try {
      ...
      if (!response.ok) {
        ...
        // throw err;
        throw {status: response.status, headers: response.headers, msg: response.statusText || '网络请求错误' };
      }
      ...
      while (true) {
        ...
        if (done) return resultHandler( response.status, responseBody, response.headers, total); // 这里处理的返回值。blob用其构造函数处理、否者拼接
